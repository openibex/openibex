...
export type OiKeyValueExtended<T = OiDbElements | OiDbArrays | OiDbObjects> = Database & {
...
/**
 * OiKeyValueExtended is based on "KeyValueIndexed", thus has similar capabilities and behavior.
 * It is extended by a "has"-method which makes querying faster.
 * 
 * OiKeyValueExtended is the recommended DB-format for databases with a high query load. 
 * 
 * @returns 
 */
export const OiKeyValueExtendedDatabase = () => async ({
  ipfs,
  identity,
  address,
  name,
...
OiKeyValueExtendedDatabase.type = type;
